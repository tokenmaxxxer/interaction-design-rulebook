#!/usr/bin/env bash
# Test harness for ux-design-cycle/hooks/state-gate.sh.
# Each case sets up a scratch "project root" with its own ux-design/state.md,
...
setup_root() {
  local dir="$1" stage="$2"
  rm -rf "$dir"
  mkdir -p "$dir/ux-design" "$dir/docs/specs"
  cp "$contract_src" "$dir/docs/specs/role-handoff-contract.md"
  if [ -n "$stage" ]; then
    cat > "$dir/ux-design/state.md" <<EOF
---
stage: $stage
...
content = "---\nstage: %s\nmetric: 7-day activation rate\nthreshold: >= 20%%\n%s---\n" % (stage, extra)
print(json.dumps({"tool_name": "Write", "tool_input": {"file_path": "ux-design/state.md", "content": content}}))
' "$2" "${3:-}"
}

# --- (a) same-state write on a state with NO self-loop row -> DENY ------
# idle has no self-loop row in transition-rules.md (only drafting|drafting
# does); this now that root resolution genuinely reads root_a's own state
# file rather than accidentally falling through to the real repo's (absent)
# state file, as it silently did before this gate honored CLAUDE_PROJECT_DIR.
root_a="$work/a"
setup_root "$root_a" "idle"
payload_a="$(json_write "$root_a" "idle" "collected_note: still gathering\n")"
...
# --- (b) drafting | drafting same-state write -> ALLOW -----------------
root_b="$work/b"
setup_root "$root_b" "drafting"
payload_b="$(json_write "$root_b" "drafting" "collected_data: 12% so far\n")"
run_gate "$root_b" "$payload_b"
code_b=$?
if [ "$code_b" -eq 0 ]; then
  pass "(b) drafting | drafting same-state write is allowed"
else
  fail "(b) drafting | drafting same-state write was DENIED (exit $code_b): $GATE_OUT"
fi

# --- (c) normal table-legal transition -> ALLOW ---------------------------
root_c="$work/c"
setup_root "$root_c" "drafting"
payload_c="$(json_write "$root_c" "drafting")"
run_gate "$root_c" "$payload_c"
code_c=$?
if [ "$code_c" -eq 0 ]; then
  pass "(c) drafting -> drafting (table-legal) is allowed"
else
  fail "(c) drafting -> drafting (table-legal) was DENIED (exit $code_c): $GATE_OUT"
fi

# --- (d) transition absent from the table -> DENY -------------------------
root_d="$work/d"
setup_root "$root_d" "idle"
payload_d="$(json_write "$root_d" "reviewed")"
run_gate "$root_d" "$payload_d"
code_d=$?
if [ "$code_d" -ne 0 ]; then
  pass "(d) idle -> reviewed (absent from table) is denied (exit $code_d)"
else
  fail "(d) idle -> reviewed (absent from table) was ALLOWED (exit 0): $GATE_OUT"
fi

# --- (e) Bash-shaped write resolving to the state file, judged same as Write
root_e="$work/e"
setup_root "$root_e" "idle"
# Use a Bash command that literally targets ux-design/state.md via a heredoc
# redirect — statically resolvable, so the gate must parse and judge it
# exactly like the Write-shaped case in (d): idle -> reviewed is illegal.
bash_cmd='cat > ux-design/state.md <<EOF
---
stage: reviewed
metric: 7-day activation rate
threshold: >= 20%
...
root_g="$work/g"
rm -rf "$root_g"; mkdir -p "$root_g/ux-design" "$root_g/docs/specs"; cp "$contract_src" "$root_g/docs/specs/role-handoff-contract.md"
cat > "$root_g/ux-design/state.md" <<'EOF'
---
stage: (none)
metric: 7-day activation rate
threshold: >= 20%
...
root_h="$work/h"
rm -rf "$root_h"; mkdir -p "$root_h/ux-design" "$root_h/docs/specs"; cp "$contract_src" "$root_h/docs/specs/role-handoff-contract.md"
cat > "$root_h/ux-design/state.md" <<'EOF'
---
stage:
metric: 7-day activation rate
threshold: >= 20%
...
root_j="$work/j"
rm -rf "$root_j"; mkdir -p "$root_j/ux-design" "$root_j/docs/specs"; cp "$contract_src" "$root_j/docs/specs/role-handoff-contract.md"
printf -- '---\r\nstage: idle   \r\nmetric: 7-day activation rate\r\nthreshold: >= 20%%\r\n---\r\n' > "$root_j/ux-design/state.md"
payload_j="$(json_write "$root_j" "drafting")"
...
root_k="$work/k"
rm -rf "$root_k"; mkdir -p "$root_k/ux-design" "$root_k/docs/specs"; cp "$contract_src" "$root_k/docs/specs/role-handoff-contract.md"
...
payload_l='{"tool_name":"Write","tool_input":{"file_path":"ux-design/state.md","content":"---\nstage: idle\nmetric: 7-day activation rate\nthreshold: >= 20%\n---\n"}}'
...
payload_q="{\"tool_name\":\"Write\",\"tool_input\":{\"file_path\":\"$non_git_target_dir/docs/reports/records/$scratch_subject_q/ux-design.md\",\"content\":\"status: idle\\n\"}}"
...
payload_r="{\"tool_name\":\"Write\",\"tool_input\":{\"file_path\":\"$target_repo_r/ux-design/state.md\",\"content\":\"---\\nstage: shipped\\nmetric: 7-day activation rate\\nthreshold: >= 20%\\n---\\n\"}}"
...
payload_m2="{\"tool_name\":\"Write\",\"tool_input\":{\"file_path\":\"docs/reports/records/$scratch_subject/ux-design.md\",\"content\":\"status: idle\\nhypothesis: docs/proposals/2026-07-26-sample.md\\n\"}}"
out_m2="$(cd "$repo_root" && printf '%s' "$payload_m2" | "$gate" 2>&1)"
code_m2=$?
if [ "$code_m2" -eq 0 ]; then
  pass "(m2) legal write to ux-design's own record slot is allowed (exit 0)"
else
  fail "(m2) legal write to ux-design's own record slot was DENIED (exit $code_m2): $out_m2"
fi
