...
trap __fc EXIT
# PreToolUse hook (Write|Edit|NotebookEdit|Bash): enforces ux-design-cycle's
...
#   1. Does this write reach ux-design/state.md (by resolved target path)?
#      If so: is the resulting `stage` transition present as a row in
#      transition-rules.md? Present -> allow. Absent -> deny.
#   2. Does this write reach ux-design's one owned kind (contract §11:
...
#   3. If the write is a `ux-design-record`
#      (`docs/reports/records/<subject>/ux-design.md`): does it carry a
#      non-empty pointer to its governing `hypothesis`/`product-record`?
#      This is the mechanically checkable half of contract §4's DEPENDS-ON
#      bullet for ux-design (depends on `hypothesis` and `product-record` —
#      the accepted problem framing its screens/flows answer). Missing ->
...
#      dependency's substance — or the design itself — is sound; what
#      counts as good design stays ux-design's own judgment, never this
#      gate's.
...
#     ux-design/state.md itself could not be read/parsed, or the hook input
#     was malformed)
...
#     ux-design role does not own, per contract §11)
#   - "missing governing-hypothesis pointer" (a ux-design-record write has
...
# into a computed name) is treated as reaching ux-design/state.md ONLY when
# it is write-shaped toward ux-design/state.md's directory; otherwise it is
# NOT globally denied — it is simply outside this gate's concern, same as
# any other unrelated write.
#
# Kill switch: export UX_DESIGN_CYCLE_OFF=1
set -uo pipefail

deny() {
  echo "ux-design-cycle: refused — $1" >&2
  exit 2
}

case "${UX_DESIGN_CYCLE_OFF:-}" in
  ""|0|false|no|off) ;;
  *) exit 0 ;;
esac
...
UX_DESIGN_ROOT="$root" UX_DESIGN_PAYLOAD="$payload" UX_DESIGN_RULES_PATH="$rules_path" python3 <<'PY'
import json
import os
import posixpath
import re
...
root = os.environ["UX_DESIGN_ROOT"]
rules_path = os.environ["UX_DESIGN_RULES_PATH"]
raw = os.environ.get("UX_DESIGN_PAYLOAD", "")
...
# ux-design owns exactly one kind under this contract (§11): the
# `ux-design-record` at `docs/reports/records/<subject>/ux-design.md`.
# Unlike product (four owned kinds: hypothesis, product-record, one-pager,
# opportunity-tree), ux-design's contract entry enforces structure only —
...
def classify_path(rel_path):
    ...
    if m:
        ...
        if record_name == "ux-design":
            return "ux-design-record", subject
...
def looks_state_shaped(token):
    ...
    ux-design/state.md (or ux-design/'s directory), even though it cannot be
    ...
    t = token.lower()
    return "state.md" in t or re.search(r'(^|/)ux-design(/|$)', t) is not None
...
def check_owned_path_write(rel_path, new_text):
    ...
    (ux-design-record must point at its governing hypothesis). Denies on
    ...
    if category == "foreign":
        deny(
            ...
            "docs/specs/role-handoff-contract.md §11. ux-design refuses to write there "
    ...
    if category == "ux-design-record":
        ...
        if not m or not m.group(1).strip():
            deny(
                "missing governing-hypothesis pointer — %s (a ux-design-record) has no "
    ...
    if isinstance(command, str) and command and _PRDD_TREE_RE.search(command):
        ...
        if not _prdd_plain_own_redirect_only:
            ...
            if not _prdd_proven_read_only:
                deny(
                    "ux-design-cycle: refused — path-reference default-deny: this Bash command "
        ...
        for op_re in BASH_WRITE_OPS:
            for m in op_re.finditer(command):
                ...
                if cand is None:
                    ...
                    # else: not write-shaped toward ux-design/state.md's
