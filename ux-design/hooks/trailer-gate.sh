...
deny() { echo "ux-design: refused — $1" >&2; exit 2; }
...
UXD_PAYLOAD="$payload" \
UXD_CPD="${CLAUDE_PROJECT_DIR:-}" \
UXD_CWD="$(pwd -P 2>/dev/null || echo)" \
python3 <<'PY'
import json, os, posixpath, re, shlex, subprocess, sys

def deny(msg):
    sys.stderr.write("ux-design: refused — %s\n" % msg)
    sys.exit(2)
...
def _uxd_fail_closed(_t, _v, _tb):
    try:
        sys.stderr.write("ux-design: refused — fail-closed: internal error (%s: %s)\n" % (_t.__name__, _v))
    except Exception:
        pass
    os._exit(2)
sys.excepthook = _uxd_fail_closed
...
cpd = os.environ.get("UXD_CPD", "")
cwd = os.environ.get("UXD_CWD", "") or "."
...
if [ "$rc" -ne 0 ] && [ "$rc" -ne 2 ]; then
  echo "ux-design: refused — fail-closed: internal error (gate judge exited $rc)" >&2
  exit 2
fi
exit "$rc"
